...
void gmain() {
    ...
    //ファイルからメモリへ読み込む
    fread(map, sizeof(char), fileSize, fp);//char(1b)*fileSize分mapに読込
    //ファイルを閉じる
    fclose(fp);
    //行数列数を数える
    int rows = 0;//行数
    ...
    int itemImg = loadImage("assets\\item.png");
    ...
    while (notQuit) {
        ...
        for (int c = startCol; c < endCol; c++) {//列数カウントアップ
            float px = -worldX + size * c;
            for (int r = 0; r < rows; r++) {//行数カウントアップ
                float py = size * r;
                int idx = r * cols + c;
                if (map[idx] == 'b') {
